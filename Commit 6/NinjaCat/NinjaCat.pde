...
void draw()
{
  ...
  {
    if (mousePressed) // If the user press on the screen then check if the user is using controls
    {
      if (!cat.win && cat.livesLeft > 0)
      {
        controlls();
      }
    }
    soundPause(myTrack, true); // Pause the music from menu
    if (levels == 1)           // If level = 1 then check if music is playing, if not then play it
    {
    ...
    {
      soundPause(level1Music, true); // Pause music from previous level
    ...
    drawBg();                  // Draws background for each level
    level1.updatelevel();      // Updates the level one
    level1.drawlevel();        // Draws the level's platform and background
    checkCollisions();         // Checks the collisions
    
    if (coinup || liveup)
    {
      if (powerUpCounter >= 40)
      {
        if (coinup)
        {
          coinup = false;
    ...
    // Remove the powerUps if drawn, after completing or losing level
    for (int i = 0; i <= objectsArray.size()-1; i++)
    {
      ...
      // Checks if user completed the level, if so display appropriate msg and reset the values
      if (draw.win)
      {
        background(255);
        fill(0);
        ...
        {
          // If user completes the last level display this msg
...
void controlls()
{
  ...
  if ((mouseX > 0+width/5 && mouseX < 0+width/3.70) && (mouseY > height - height/5 && mouseY < height-height/15))
  {
    ...
    // Increments cat's position, decrement the platform
  ...
  } else if ((mouseX > width * 0.87890625 && mouseX < width * 0.9765625) && (mouseY > height * 0.59027777777778 && mouseY < height * 0.7638)) // Check if user clicked on "jump right" control
  {
    ...
    {
      cat.goup = !cat.goup;  // change the boolean to true which will allow the cat to jump
      soundPlay(jumpMusic);  // play jump sound
    }

    // Decrements the position of powerUps
    for (int i = 0; i <= objectsArray.size()-1; i++)
    {
      BaseClass draw = objectsArray.get(i);
...
void mousePressed()
{
  /*This method checks if user is in menu or is playing the game, if user is playing the game then this function will allow him to shoot.
   The shoot method is implemented in void mousePressed() in order to prohibit constant shooting.
   If the user is in menu, this method will check if user clicked on particular tab (e.g. "menu", "keyboard", "exit") and take appropriate action.
   */


  // If levels > 0, that means user is no longer in menu
  if (levels > 0)
  {
  ...
  } else if (levels == 0 && mouseY > height-height/10)   // If levels == 0, that means if the Menu is showed
  {
    if (mouseX < width/3)
    {
...
void keyPressed()
{
  /* This method checks if the keyboard is toggled, if so then allow the user to "talk" to the cat */
...
void checkCollisions()
{
  for (int i = objectsArray.size() - 1; i >= 0; i --)
  {
    ...
    if (theCat instanceof Cat)
    {
      ...
      {
        BaseClass life = objectsArray.get(j);
        if (life instanceof Lives || life instanceof Coin) // Check if life is one of the powerUps 
        {
          if (theCat.pos.x >= (life.livesx - width/15) && theCat.pos.x <= (life.livesx + width/15)) // check the collision
          {
...
void powerUpfxn()
{
  /* This method displays text beside the cat, when collecting the powerUp */
